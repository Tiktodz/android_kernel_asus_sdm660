...
extern void snvt_change_mode(uint8_t mode);
extern uint8_t snvt_get_fw_pipe(void);
extern void snvt_read_mdata(uint32_t xdata_addr, uint32_t xdata_btn_addr);
extern void snvt_get_mdata(int32_t *buf, uint8_t *m_x_num, uint8_t *m_y_num);
...
static void nvt_print_data_log_in_one_line(int32_t *data, int32_t data_num)
{
	...
	if (!tmp_log) {
		SNVT_ERR("kzalloc for tmp_log failed!\n ");
		return;
	}
...
static int32_t nvt_polling_hand_shake_status(void)
{
	...
	if (i >= retry) {
		SNVT_ERR("polling hand shake status failed, buf[1]=0x%02X\n", buf[1]);
		...
		SNVT_ERR("Read back 5 bytes from offset EVENT_MAP_HOST_CMD: 0x%02X, 0x%02X, 0x%02X, 0x%02X, 0x%02X\n", buf[1], buf[2], buf[3], buf[4], buf[5]);
...
static int8_t nvt_switch_FreqHopEnDis(uint8_t FreqHopEnDis)
{
	...
	if (unlikely(retry == 20)) {
		SNVT_ERR("switch FreqHopEnDis 0x%02X failed, buf[1]=0x%02X\n", FreqHopEnDis, buf[1]);
		ret = -1;
	}
...
static int32_t nvt_read_baseline(int32_t *xdata)
{
	...
	snvt_get_mdata(xdata, &x_num, &y_num);
...
static void nvt_enable_noise_collect(int32_t frame_num)
{
	...
	buf[3] = frame_num;
	buf[4] = 0x00;
	SCTP_I2C_WRITE(nts->client, SI2C_FW_Address, buf, 5);
}
...
#endif /* #if STOUCH_KEY_NUM > 0 */
	if (!rawdata_buf) {
		SNVT_ERR("kzalloc for rawdata_buf failed!\n");
		return -ENOMEM;
	}

	if (snvt_get_fw_pipe() == 0)
		raw_pipe_addr = nts->mmap->RAW_PIPE0_ADDR;
	else
		raw_pipe_addr = nts->mmap->RAW_PIPE1_ADDR;

	for (y = 0; y < IC_Y_CFG_SIZE; y++) {
		//---change xdata index---
		snvt_set_page(SI2C_FW_Address, raw_pipe_addr + y * IC_X_CFG_SIZE * 2);
		buf[0] = (uint8_t)((raw_pipe_addr + y * IC_X_CFG_SIZE * 2) & 0xFF);
		SCTP_I2C_READ(nts->client, SI2C_FW_Address, buf, IC_X_CFG_SIZE * 2 + 1);
		memcpy(rawdata_buf + y * IC_X_CFG_SIZE * 2, buf + 1, IC_X_CFG_SIZE * 2);
	}
#if STOUCH_KEY_NUM > 0
	if (snvt_get_fw_pipe() == 0)
		raw_btn_pipe_addr = nts->mmap->RAW_BTN_PIPE0_ADDR;
	else
		raw_btn_pipe_addr = nts->mmap->RAW_BTN_PIPE1_ADDR;

	//---change xdata index---
	snvt_set_page(SI2C_FW_Address, raw_btn_pipe_addr);
	buf[0] = (uint8_t)(raw_btn_pipe_addr & 0xFF);
	SCTP_I2C_READ(nts->client, SI2C_FW_Address, buf, IC_KEY_CFG_SIZE * 2 + 1);
...
static int32_t nvt_read_fw_short(int32_t *xdata)
{
	...
	for (y = 0; y < Y_Channel; y++) {
		//---change xdata index---
		snvt_set_page(SI2C_FW_Address, raw_pipe_addr + y * X_Channel * 2);
		buf[0] = (uint8_t)((raw_pipe_addr + y * X_Channel * 2) & 0xFF);
		SCTP_I2C_READ(nts->client, SI2C_FW_Address, buf, X_Channel * 2 + 1);
	...
	snvt_set_page(SI2C_FW_Address, raw_btn_pipe_addr);
	buf[0] = (uint8_t)(raw_btn_pipe_addr & 0xFF);
	SCTP_I2C_READ(nts->client, SI2C_FW_Address, buf, Key_Channel * 2 + 1);
...
{
	struct device_node *np = nts->client->dev.of_node;
	...
	if(of_property_read_bool(np, "novatek,mp-support-dt")) {
		 ...
		 * The string rule is "novatek-mp-criteria-<snvt_pid>"
		 * snvt_pid is 2 bytes (show hex).
		 *
		 * Ex. snvt_pid = 500A
		 *     mpcriteria = "novatek-mp-criteria-500A"
		 */
		snprintf(mpcriteria, sizeof(mpcriteria), "novatek-mp-criteria-%04X", nts->snvt_pid);

		if (nvt_mp_parse_dt(np, mpcriteria)) {
			mutex_unlock(&nts->lock);
			SNVT_ERR("mp parse device tree failed!\n");
			return -EINVAL;
		}
	} else {
		SNVT_LOG("Not found novatek,mp-support-dt, use default setting\n");
		//---Print Test Criteria---
		nvt_print_criteria();
	}
	...
	if (nvt_switch_FreqHopEnDis(FREQ_HOP_DISABLE)) {
		mutex_unlock(&nts->lock);
		SNVT_ERR("switch frequency hopping disable failed!\n");
		return -EAGAIN;
	}

	if (snvt_check_fw_reset_state(RESET_STATE_NORMAL_RUN)) {
		mutex_unlock(&nts->lock);
		SNVT_ERR("check fw reset state failed!\n");
		return -EAGAIN;
	}
	...
	if (snvt_clear_fw_status()) {
		mutex_unlock(&nts->lock);
		SNVT_ERR("clear fw status failed!\n");
		return -EAGAIN;
	}

	snvt_change_mode(MP_MODE_CC);

	if (snvt_check_fw_status()) {
		mutex_unlock(&nts->lock);
		SNVT_ERR("check fw status failed!\n");
		return -EAGAIN;
	}
...
int32_t nvt_mp_parse_ain(struct device_node *np, const char *name, uint8_t *array, int32_t size)
{
	...
	if ((!data) || (!len) || (len != size)) {
		SNVT_ERR("error find %s. len=%d\n", name, len);
		return -1;
	} else {
		SNVT_LOG("%s. len=%d\n", name, len);
		ret = of_property_read_u32_array(np, name, tmp, len);
		if (ret) {
			SNVT_ERR("error reading %s. ret=%d\n", name, ret);
			return -1;
		}
...
int32_t nvt_mp_parse_dt(struct device_node *root, const char *node_compatible)
{
	...
	SNVT_LOG("Parse mp criteria for node %s\n", node_compatible);

	/* find each MP sub-nodes */
	for_each_child_of_node(root, child) {
		/* find the specified node */
		if (of_device_is_compatible(child, node_compatible)) {
			SNVT_LOG("found child node %s\n", node_compatible);
			np = child;
			break;
		}
	}
	if (child == NULL) {
		SNVT_ERR("Not found compatible node %s!\n", node_compatible);
		return -1;
	}
	...
	SNVT_LOG("Parse mp criteria done!\n");
...
int32_t nvt_mp_proc_init(void)
{
	...
	if (NVT_proc_selftest_entry == NULL) {
		SNVT_ERR("create /proc/nvt_selftest Failed!\n");
		return -1;
	} else {
		if(nvt_mp_buffer_init()) {
			SNVT_ERR("Allocate mp memory failed\n");
			return -1;
		}
		else {
			SNVT_LOG("create /proc/nvt_selftest Succeeded!\n");
		}
		return 0;
	}
}
